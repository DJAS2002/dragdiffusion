...
def get_features_plus_reduce(feature, position, reduce_dims=()):
    # arg: reduce_dims: tuple
    #   if reduce_dims is empty: no aggregation
    #   else: reduce dims via summation
    # feature: (1,C,H,W)
    # position: (N,2)
    # return: (N,C)
    ...
    return output.sum(dim=reduce_dims) if len(reduce_dims) > 0 else output
...
def get_each_point(current, target_final, L, feature_map, max_distance, template_feature,
    ...
    if loss_end < threshold_l:
        ...
        # features all has shape (interval_number * neighbourhood patch size, channels)
        # where several patches are concatenated together. Therefore, in the case of feature aggregation,
        # we need to aggregate manually by reshaping to (interval, neighbourhood patch size, channels)
        # and then performing aggregation over some dims
        features_all = get_features_plus_reduce(feature_map, candidate_points_local, reduce_dims=())
        ...
        target_max = current + min(2 * d_max / (d_remain + 1e-8), 1) * (
                    target_final - current)  # double the localization range
...
def freedrag_update(model,
    ...
    get_features_plus = lambda feature, position: get_features_plus_reduce(feature, position, reduce_dims=args.reduce_dims)
    ...
    # print(F_template[0].shape, F0.shape, current_targets[0, :].shape, position_local.shape)
        ...
        for step in range(5):
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                unet_output, F1 = model.forward_unet_features(init_code, t, encoder_hidden_states=text_emb,
                                                              ...
                                                              interp_res_h=args.sup_res_h, interp_res_w=args.sup_res_w)
...
def freedrag_update_gen(model,
    ...
    get_features_plus = lambda feature, position: get_features_plus_reduce(feature, position,
